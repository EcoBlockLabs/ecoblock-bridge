...
export function SwitchNetworksButton(
  ...
  return (
    <button
      ...
      {...props}
    >
      <SwitchVerticalIcon className="arbitrum-switch text-dark" />
    </button>
  )
}
...
function NetworkListbox({
  ...
  return (
    ...
    >
      <Listbox.Button
        className={`arb-hover flex w-max items-center space-x-1 rounded-full px-2 py-2 text-sm text-white md:text-lg lg:px-2 lg:py-2 ${buttonClassName}`}
      >
        <span>
...
function BalancesContainer({ children }: { children: React.ReactNode }) {
  return (
    <div className="ml-1 flex flex-col flex-nowrap items-start break-all text-sm font-extralight tracking-[.25px] text-white md:items-end md:text-base lg:font-medium lg:uppercase">
      {children}
    </div>
  )
}
